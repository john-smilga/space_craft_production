import type { GridResponse, LayoutItem, AvailableItem } from '@/types/planograms';
...
export function usePlanogramLayout() {
  const { gridData, rowLayouts, setRowLayouts } = usePlanogramLayoutStore();
  const { availableItems, selectedAvailableItems, targetRowId, clearSelectedAvailableItems, closeAvailableProductsSidebar } = usePlanogramAvailableProductsStore();
