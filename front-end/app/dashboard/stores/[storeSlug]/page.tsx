...
export default function StoreDetailPage() {
  ...
  const { data: projectsData } = useFetch<ProjectsResponse>('/projects/');
  ...
  const deleteMutation = useMutation<void, void>(
    ...
    { toastResource: 'store', operation: 'delete' }
  );
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 gap-4 gap-y-8'>
              ...
              <AdminOnly>
                <div className='md:col-start-4 md:row-start-2 flex flex-row gap-2'>
                  ...
                  <Button onClick={handleDelete} disabled={deleteMutation.loading} variant={deleteConfirm ? 'destructive' : 'outline'} size='sm' className={deleteConfirm ? '' : 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100 hover:text-red-800'}>
                    {deleteMutation.loading ? 'Deleting...' : deleteConfirm ? 'Confirm Delete' : 'Delete Store'}
                  </Button>
                  {deleteConfirm && (
