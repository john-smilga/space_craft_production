...
class Planogram(models.Model):
    ...
    project = models.ForeignKey(
        "projects.Project", on_delete=models.CASCADE, related_name="planograms"
    )
    company = models.ForeignKey(
        "accounts.Company", on_delete=models.CASCADE, related_name="planograms"
    )
    ...
    width_in = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Display width in inches"
    )
    height_in = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Display height in inches"
    )
    ...
    category_ids = models.JSONField(
        default=list, help_text="List of selected category IDs"
    )
