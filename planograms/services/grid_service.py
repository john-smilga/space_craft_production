...
def layout_by_score(
    ...
    *,
    ...
    logger.info(f"=== layout_by_score CALLED ===")
    ...
    logger.info(f"Items per category: {[(cat, len(items)) for cat, items in products_by_category.items()]}")
    
    if num_categories == 0:
        # No categories - return empty rows
        rows_payload = []
        for r in range(rows_cnt):
            rows_payload.append({"id": r + 1, "category": None, "name": f"Shelf {r + 1}", "items": []})
        return {
    ...
    for i, cat in enumerate(ranked_cats):
        ...
        logger.info(f"Category '{cat}' assigned columns {start_col} to {end_col-1} ({category_cols} cols)")
        start_col = end_col
    ...
    for cat in ranked_cats:
        ...
        if category_items:
            ...
            sorted_by_score = sorted(category_items, 
                                   key=lambda p: p.get('overall_score', 0.0), 
                                   reverse=True)
            highest_score_products_per_category[cat] = sorted_by_score[0].get('id')
            logger.info(f"Category '{cat}' highest score product ID: {highest_score_products_per_category[cat]}")
    
    # Track how many times highest score products have been used globally
    highest_score_product_counts = {cat: 0 for cat in highest_score_products_per_category.keys()}
    ...
    for r in range(rows_cnt):
        ...
        for cat_range in category_column_ranges:
            ...
            while placed_this_iteration and cursor_x < section_end:
                ...
                if highest_score_placed_in_row and highest_score_product_id and highest_score_product_counts[cat] < 2:
                    # Find the highest score product in available_products
                    highest_score_product = next((p for p in available_products if p.get('id') == highest_score_product_id), None)
                    if highest_score_product:
                        # Remove it from the list
                        available_products.remove(highest_score_product)
                        # Sort other products first
                        available_products.sort(key=lambda p: (p.get('margin', 0.0), p.get('overall_score', 0.0)), reverse=True)
                    ...
                    if cursor_x + w_cells <= section_end:
                        ...
                        if product_id == highest_score_product_id:
                            ...
                            if not highest_score_placed_in_row:
                                ...
                                highest_score_first_x = cursor_x - w_cells  # Store the x position where it was placed
