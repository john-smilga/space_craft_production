...
from planograms.services.grid_service import layout_by_score, compute_grid_geometry
...
class GridServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.products_category_a = [
            {"id": 1, "name": "Product A1", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.8, "overall_score": 0.9},
            {"id": 2, "name": "Product A2", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.7, "overall_score": 0.8},
            {"id": 3, "name": "Product A3", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.6, "overall_score": 0.7},
            {"id": 4, "name": "Product A4", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.5, "overall_score": 0.6},
            {"id": 5, "name": "Product A5", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.4, "overall_score": 0.5},
        ]
        
        self.products_category_b = [
            {"id": 10, "name": "Product B1", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.85, "overall_score": 0.95},
            {"id": 11, "name": "Product B2", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.75, "overall_score": 0.85},
            {"id": 12, "name": "Product B3", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.65, "overall_score": 0.75},
            {"id": 13, "name": "Product B4", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.55, "overall_score": 0.65},
        ]
    ...
    def test_column_based_structure(self):
        ...
        grid = compute_grid_geometry(
            shelf_width_in=60.0,  # 10 cells (60/6)
        ...
        for row in layout["rows"]:
            if len(row["items"]) > 0:  # Only check rows that have items
                ...
                self.assertEqual(len(categories_in_row), 2,
    ...
    def test_different_products_in_different_rows(self):
        ...
        extra_products = [
            {"id": 6, "name": "Product A6", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.3, "overall_score": 0.4},
            {"id": 7, "name": "Product A7", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.2, "overall_score": 0.3},
            {"id": 8, "name": "Product A8", "pack_width_in": 6.0, "pack_height_in": 3.0, 
             "margin": 0.1, "overall_score": 0.2},
        ]
        ...
        all_row_product_ids = []
        for row in layout["rows"]:
            row_product_ids = set(item["meta"]["id"] for item in row["items"] 
        ...
        if len(all_row_product_ids) >= 2:
            ...
            self.assertGreater(unique_sets_count, 1,
                             "Different rows should have different products")
    ...
    def test_products_sorted_by_margin(self):
        ...
        row = layout["rows"][0]
        category_a_items = [item for item in row["items"] 
                           if item["meta"]["category"] == "category_a"]
        ...
        if len(category_a_items) > 1:
            ...
            if 1 in product_ids and 2 in product_ids:
                ...
                self.assertLess(idx1, idx2, 
                              "Higher margin products should appear first")
    ...
    def test_column_division_equal_sections(self):
        ...
        category_a_items = [item for item in row["items"] 
                           if item["meta"]["category"] == "category_a"]
        category_b_items = [item for item in row["items"] 
                           if item["meta"]["category"] == "category_b"]
    ...
    def test_no_repetition_across_categories_in_row(self):
        ...
        products_by_category = {
            "category_a": [{"id": 1, "name": "Product 1", "pack_width_in": 6.0, 
                           "pack_height_in": 3.0, "margin": 0.8, "overall_score": 0.9}],
            "category_b": [{"id": 1, "name": "Product 1", "pack_width_in": 6.0, 
                           "pack_height_in": 3.0, "margin": 0.7, "overall_score": 0.8}],
        }
    ...
    def test_products_without_width_skipped(self):
        ...
        products_by_category = {
            "category_a": [
                {"id": 1, "name": "Product 1", "pack_width_in": 6.0, 
                 "pack_height_in": 3.0, "margin": 0.8, "overall_score": 0.9},
                {"id": 2, "name": "Product 2", "pack_width_in": None, 
                 "pack_height_in": 3.0, "margin": 0.7, "overall_score": 0.8},
                {"id": 3, "name": "Product 3", "pack_width_in": 6.0, 
                 "pack_height_in": 3.0, "margin": 0.6, "overall_score": 0.7},
            ],
        }
    ...
    def test_bulky_items_only_on_bottom_rows(self):
        ...
        products_by_category = {
            "category_a": [
                {"id": 1, "name": "Regular Product", "pack_width_in": 6.0, 
                 "pack_height_in": 3.0, "margin": 0.8, "overall_score": 0.9},
                {"id": 2, "name": "Bulky Product", "pack_width_in": 6.0, 
                 "pack_height_in": 6.0, "margin": 0.7, "overall_score": 0.8},  # Bulky
            ],
        }
        ...
        self.assertTrue(bulky_in_bottom or len(row2_product_ids) == 0,
    ...
    def test_multiple_rows_unique_products(self):
        ...
        all_product_ids = []
        for row in layout["rows"]:
            row_product_ids = [item["meta"]["id"] for item in row["items"] 
                              if item["meta"]["category"] == "category_a"]
        ...
        if len(layout["rows"]) > 1:
            row1_ids = set(item["meta"]["id"] for item in layout["rows"][0]["items"] 
                          if item["meta"]["category"] == "category_a")
            row2_ids = set(item["meta"]["id"] for item in layout["rows"][1]["items"] 
                          if item["meta"]["category"] == "category_a")
    ...
    def test_highest_score_product_repeats_once(self):
        ...
        self.assertEqual(product_counts[highest_score_product_id], 2,
        ...
        for row in layout["rows"]:
            row_items = [item for item in row["items"] 
                        if item["meta"]["category"] == "category_a"]
        ...
        self.assertTrue(found_side_by_side,
