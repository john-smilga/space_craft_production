...
from .data.categories import get_category_path
from .data.category_hierarchy import CATEGORY_HIERARCHY, get_category_path_by_id


def _flatten_products_by_category(category_dict: Dict, path_parts: List[str] = None) -> Dict[str, List[Dict]]:
    """
...
def get_products_for_categories(
    ...
    for category_id in category_ids:
        ...
        for product in all_products_by_path[product_lookup_path]:
            ...
            seasonal_metrics = PRODUCT_SEASONAL_METRICS.get(product_id, {}).get(season, {})
...
def get_children_by_category_id(category_id: Optional[int] = None) -> Dict[str, Any]:
    """
...
def _get_products_by_path(category_path: str, season: str = "summer") -> List[Dict[str, Any]]:
...
def get_product_by_id(product_id: int, season: str = "summer") -> Optional[Dict[str, Any]]:
    """
