...
class Display(models.Model):
    ...
    shelf_spacing = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    display_category = models.CharField(max_length=20, choices=DISPLAY_CATEGORY_CHOICES, default='custom', help_text="Standard displays are available to all companies, custom displays are company-specific")
    company = models.ForeignKey('accounts.Company', on_delete=models.CASCADE, null=True, blank=True, related_name='displays', help_text="Null for standard displays, set for custom displays")
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='created_displays')
